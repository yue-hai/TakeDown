...
```shell
D:\app\devTools\Environments\Web\
...
2. 解压到 `D:\app\devTools\Environments\Web\nvm-noinstall` 目录

### ②、创建 settings.txt

> 需要使用 VS Code 保存为 UTF-8；记事本默认 Unicode(UTF-16) 会导致 nvm 读不到配置，导致 nvm root 为空

1. 在 `D:\app\devTools\Environments\Web\nvm-noinstall` 目录下创建 `settings.txt`，必须是 UTF-8，输入以下内容：

```shell
root: D:\app\devTools\Environments\Web\nvm-noinstall
path: D:\app\devTools\Environments\Web\nvm-nodejs
arch: 64
proxy: none
```
...
| 变量名         | 值                                              |
| ----------- | ---------------------------------------------- |
| NVM_HOME    | D:\app\devTools\Environments\Web\nvm-noinstall |
| NVM_SYMLINK | D:\app\devTools\Environments\Web\nvm-nodejs    |

2. 添加PATH 配置：
	1. `%NVM_HOME%`，用于 nvm 命令
3. 不要把 `%NVM_SYMLINK%` 或 `D:\app\devTools\Environments\Web\nvm-nodejs` 目录加入 PATH
...
nvm root       # 期望输出：D:\app\devTools\Environments\Web\nvm-noinstall
...
1. 安装位置：`D:\app\devTools\Environments\Web\nvm-noinstall\v14.17.0`
2. 不要执行 `nvm use 14.17.0`，会改 nvm 软链接，不符合全局不变方案
3. `D:\app\devTools\Environments\Web\nvm-nodejs` 目录为空是正常的
4. nvm 安装的是 `node64.exe`，没有 node.exe，需手动复制一次：

```shell
copy D:\app\devTools\Environments\Web\nvm-noinstall\v14.17.0\node64.exe D:\app\devTools\Environments\Web\nvm-noinstall\v14.17.0\node.exe
```
...
```shell
$env:PATH = "D:\app\devTools\Environments\Web\nvm-noinstall\v14.17.0;$env:PATH"
```
