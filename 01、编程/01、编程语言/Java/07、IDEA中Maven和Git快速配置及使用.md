...
![](attachments/03、配置环境变量1.png)

![](attachments/04、配置环境变量2.png)
...
![](attachments/02、Idea自带的Maven在Idea的安装目录的plugins目录中.png)

2. 在自带的 Maven 里配置了本地仓库之后，打开 Idea 配置为我们自己配置的 Maven

![](attachments/05、配置我们自己安装的Maven.png)

3. 设置 Maven 自动导包

![](attachments/06、设置Maven自动导包.png)

# 四、在 Idea 中创建 Maven 项目
1. 创建 Java 工程

![](attachments/07、创建Java工程.png)

2. 点击 Next，配置要继承的模块（如果直接创建的是 Project 不存在这一项）、坐标（GAV）、路径。不同的 Idea 版本可能有所差别，我使用的是 2019.3.3的版本

![](attachments/08、创建Java工程2.png)
...
![](attachments/09、配置Maven的核心配置文件pom.xml.png)

6. 使用 Maven 的方式运行 Maven 工程

![](attachments/10、使用Maven的方式运行Maven工程.png)

# 五、创建 Web 工程（了解）

1. 创建简单的 Maven 工程，打包方式为 war 包

![](attachments/11、创建简单的Maven工程.png)
...
![](attachments/12、点击ProjectStructure.png)

3. 选择对应的 Module，设置 Web 目录

![](attachments/13、选择对应的Module设置Web目录.png)

![](attachments/14、选择对应的Module设置Web目录2.png)

4. 弹出提示框，选择版本后点击 OK

![](attachments/15、弹出提示框选择版本后点击OK.png)

5. 生成 web.xml 文件

![](attachments/16、生成web.xml文件.png)

6. 设置存放 web 页面文件的目录后点击 OK

![](attachments/17、设置存放页面的目录的位置和名称.png)

7. 发现项目中多了一个 web 目录，而且目录上有一个蓝点

![](attachments/18、发现项目中多了一个web目录而且目录上有一个蓝点.png)

8. 在 web 目录下创建 index.jsp 页面

![](attachments/19、在web目录下创建index.jsp页面.png)
...
![](attachments/20、点击ProjectStructure.png)

2. 点击 Modules→➕→Import Module

![](attachments/21、点击%20Modules→➕→Import%20Module.png)

3. 找到项目所在的位置

![](attachments/22、找到项目所在的位置.png)

4. 选择 Import module from external model（从外部模型导入模块）→Maven→Finish

![](attachments/23、选择%20Import%20module%20from%20external%20model（从外部模型导入模块）→Maven→Finish.png)
