...
Vault absolute path: D:\OneDrive\内存\文档资料\笔记\TakeDown
...
- ✗ WRONG: `/notes/my-note.md`, `D:\OneDrive\内存\文档资料\笔记\TakeDown/file.md`
...
- Located in media folder: `././attachments`
- Read with: `Read file_path="./attachments/image.jpg"`
...
curl -sfo "./attachments/$img_name" 'URL'
```

Then read with `Read file_path="./attachments/$img_name"`, and replace the markdown link `![alt](url)` with `![[./attachments/$img_name]]` in the note.

**Benefits**: Image becomes a permanent vault asset, works offline, and uses Obsidian's native embed syntax.

## Custom Instructions

除非有我的允许，否则任何情况下都不要改动任何文件
如果要改动任何文件，都必须额外的征求我的同意，我明确同意了才可以进行改动
