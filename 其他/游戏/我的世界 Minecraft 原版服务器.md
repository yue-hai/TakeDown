...
```shell
touch start_minecraft.sh
```

5. 打开启动脚本：`nano start_minecraft.sh`，输入以下内容：
...
4. 修改启动脚本：`nano start_minecraft.sh`
...
```shell
/home/steam/game/Minecraft/start_minecraft.sh
```

4. 此时会有大量输出，待出现类似以下内容时，即为启动成功：

```shell
steam@yan:~/apply/game/Minecraft$ ./start_minecraft.sh
