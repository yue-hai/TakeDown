...
2. 选择轻量型服务器，1 核 2G 就勉强使用，2 核 4G 是推荐配置
    - 地域：随意
    - 镜像类型：系统镜像
...
2. 进入 steam 用户，创建 `steamcmd` 和 `7DaysToDieServer	` 文件夹，再进入 `steamcmd` 目录

```bash
su steam && mkdir steamcmd && mkdir 7DaysToDieServer && cd steamcmd
```
...
3. 获取 screen 列表，会有一个 `XXX.master` 和 `XXX.caves` 的进程
...
1. 服务端主目录：`~/steam/7DaysToDieServer`
2. 服务端地图目录：`~/steam/7DaysToDieServer/Data/Worlds`
3. 服务端存档目录：`~/.local/share/7DaysToDie/Saves`
4. 服务器日志输出目录：`~/steam/7DaysToDieServer/7DaysToDieServer_Data`
5. Mods 目录，需要自己创建：`~/steam/7DaysToDieServer/Mods`
6. 服务器配置文件：`~/steam/7DaysToDieServer/serverconfig.xml`
7. 管理员配置文件：`~/.local/share/7DaysToDie/Saves/serveradmin.xml` 

# 八、添加 mod

1. 进入七日杀主目录：`~/steam/7DaysToDieServer`
2. 创建目录 Mods
...
3. 将想要使用的 mod 上传至 Mods 目录即可，游戏会自动加载
4. mod 目录的名字不要使用中文
