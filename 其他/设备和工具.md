...
> 这个【设置自动关机】和下面的【自定义事件】选择其中一个即可
> 
> 个人推荐使用【自定义事件】

1. NUT 默认会在 UPS 发送LOWBATT时通知机器关机，需配置 `/etc/nut/upsmon.conf` 文件，在其中添加：
...
3. 在此设置的是当主机使用 UPS 电池供电时写入日志并运行之前定义的 upssched 服务。
4. NUT 除 ONBATT 还提供了多种状态，具体内容可在 `upsmon.conf` 文件的注释中查到
...
# 指定了 upssched 使用的管道文件的路径。在 NUT 系统中，upssched 通过管道与其他进程>
# 这里设置了管道文件的位置为 /var/run/nut/upssched/upssched.pipe，通常用于传递事件>
PIPEFN /var/run/nut/upssched/upssched.pipe
# 指定了锁文件的位置。锁文件用于确保同时只有一个 upssched 实例在处理事件，防止多次>
# 这里的锁文件路径为 /var/run/nut/upssched/upssched.lock
LOCKFN /var/run/nut/upssched/upssched.lock

# 定义了当 UPS 切换到电池供电（ONBATT）时的行为：
# ONBATT *：当任何（*）UPS 设备进入电池供电模式时触发
# EXECUTE on-battery：立即将 on-battery 作为参数传递给 CMDSCRIPT 指定的脚本
AT ONBATT * EXECUTE on-battery
# 定义了在 UPS 切换到电池供电（ONBATT）时的行为：
# START-TIMER power-off 60：启动一个名为 power-off 的定时器，在 60s 后触发，并将 p>
AT ONBATT * START-TIMER power-off 60
# 定义了当 UPS 恢复到市电供电（ONLINE）时的行为：
# ONLINE *：当任何（*）UPS 设备恢复市电时触发
# CANCEL-TIMER power-off：取消之前启动的名为 power-off 的计时器
AT ONLINE * CANCEL-TIMER power-off
# 定义了在 UPS 恢复市电时的另一个操作：
# EXECUTE power-on：立即将 power-on 作为参数传递给 CMDSCRIPT 指定的脚本
...
path_log="/home/yan/apply/tools/SANTAK-TGBOX-850-NUT/SANTAK-TGBOX-850-NUT.log"
...
case $1 in
    ...
    on-battery)
        # 记录日志
        echo "【$(date '+%Y-%m-%d %H:%M:%S')】 - ups 供电已中断" >> $path_log
        # 发送邮件通知
        echo "【$(date '+%Y-%m-%d %H:%M:%S')】 - ups 供电已中断" | mail -s "ups 供电已中断" -r yan@yue-hai.top 770717410@qq.com
    ;;
    # 当接收到 power-off 参数时，表示电源已变为 ONBATT 状态，此时 UPS 转为电池供电
    # 60s 后调用该脚本，传递参数 power-off，服务器将记录日志并、发送邮件通知、关机
    power-off)
        # 记录日志
        echo "【$(date '+%Y-%m-%d %H:%M:%S')】 - ups 供电已中断超过 60s，即将关机" >> $path_log
        # 发送邮件通知
        echo "【$(date '+%Y-%m-%d %H:%M:%S')】 - ups 供电已中断超过 60s，即将关机" | mail -s "ups 供电已中断超过 60s" -r yan@yue-hai.top 770717410@qq.com
        # 延迟 1 分钟后关机；也就是说在断电后 2 分钟内 UPS 未恢复供电，服务器将关机
        shutdown -h +1 "由于 UPS 断电，系统将在 1 分钟内关闭"
    ;;
    # 当接收到 power-on 参数时，表示电源已变为 ONLINE 状态，此时 UPS 转为市电供电
    # 服务器将记录日志并发送邮件通知
    power-on)
        # 记录日志
        echo "【$(date '+%Y-%m-%d %H:%M:%S')】 - ups 供电已恢复" >> $path_log
        # 发送邮件通知
        echo "【$(date '+%Y-%m-%d %H:%M:%S')】 - ups 供电已恢复" | mail -s "ups 供电已恢复" -r yan@yue-hai.top 770717410@qq.com
        # 电源恢复后，取消关机计划；也就是说只要 UPS 在断电后 2 分钟内恢复供电，服务器将取消关机计划
        shutdown -c
    ;;
    # 当接收到未知参数时，记录日志
    *)
        echo "【$(date '+%Y-%m-%d %H:%M:%S')】 - 未知参数：$1" >> $path_log
    ;;
esac
```

3. 类似的，还可以通过编写脚本的方式进行其他操作
