...
2. 在 Termux 中执行以下命令，开启正则过滤并监听 V8 引擎创建 JS Context 时的日志路由记录：

```shell
logcat | grep -iE "corpid=ding[a-zA-Z0-9]+"
```

3. 将 Termux 保持后台运行，返回桌面并打开钉钉
4. 走一遍完整的手动打卡流程：主界面 -> 工作台 -> 考勤打卡 -> 弹窗中选择要打卡的特定公司 -> 页面完全加载完毕
5. 切回 Termux，终止日志监听（Ctrl + C，或者直接看屏幕输出），终端内应该已捕获到类似如下的日志：
...
6. 其中 `corpId=` 后面的 `dingda973a41gneodk22r6qfd3l5ga4s3dy6` 就是 组织 id
