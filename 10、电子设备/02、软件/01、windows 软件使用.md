...
> ltsc 等系统没有微软商店的安装方式：[4、ltsc 安装 `.msixbundle` 或 `.msix` 应用包](#10-1)
...
![|500](attachments/Pasted%20image%2020250911094545.png)
...
![|475](attachments/Pasted%20image%2020250911094939.png)

3. 先开启 Mixin，再开启系统代理

![|475](attachments/Pasted%20image%2020250911095128.png)

4. 最后在 代理 -> 规则中选择节点即可，<font color="#c00000">不要在全局中选择</font>

![|475](attachments/Pasted%20image%2020250911095151.png)
...
![|500](attachments/Pasted%20image%2020240107201712.png)

2. 点击高级

![|500](attachments/Pasted%20image%2020240107201738.png)

3. 在弹出的页面中添加参数：`--initial_prompt 以下是普通话的句子`

![|500](attachments/Pasted%20image%2020240107201850.png)
...
![|425](attachments/Pasted%20image%2020240315100518.png)
...
![|475](attachments/Pasted%20image%2020240709091724.png)

4. 再进入 `plugins` 目录，没有的话自行创建

![|475](attachments/Pasted%20image%2020240709091816.png)

5. 根据插件的名字创建一个目录，用于存放刚才下载的插件，比如对应刚才下载的 `editing-toolbar`

![|475](attachments/Pasted%20image%2020240709091942.png)

6. 将刚才下载的 `main.js`、`manifest.json`、`styles.css` 复制到 `editing-toolbar` 目录中

![|475](attachments/Pasted%20image%2020240709092200.png)

7. 打开 obsidian 软件，进入设置，选择第三方插件，启用对应的插件即可

![|475](attachments/Pasted%20image%2020240709092448.png)
...
![|500](attachments/Pasted%20image%2020240709090758.png)
...
![|428](attachments/Pasted%20image%2020240709091338.png)
...
![|375](attachments/mousewheel-image-zoom.gif)
...
![|425](attachments/quiet-outline-1.gif)

![|425](attachments/quiet-outline-2.gif)
...
![|404](attachments/Pasted%20image%2020241024144247.png)

2. 启用后，返回 obsidian 的设置，选择 Text Generator，进入其设置页面

![|25](attachments/Pasted%20image%2020241024144431.png)
...
![|475](attachments/Pasted%20image%2020241024142952.png)

5. 点击模型名进入详情，点击 API，然后点击 Create APl key 创建 key

![|475](attachments/Pasted%20image%2020241024143122.png)

6. 在新页面中，再次点击 Create APl key，输入名称点击 Create 即可

![|406](attachments/Pasted%20image%2020241024143250.png)
...
![|550](attachments/Pasted%20image%2020241024144431.png)

3. 在 Provider Profile 中选择 Custom，然后点击右侧的 +

![|550](attachments/Pasted%20image%2020241024145628.png)

4. 在 Endpoint 中，填入请求地址，即上面的：https://openrouter.ai/api/v1/chat/completions

![|550](attachments/Pasted%20image%2020241024145913.png)

5. 在 API Key 中填入 api_key，即上面的：`sk-or-v1-12......`

![|550](attachments/Pasted%20image%2020241024150718.png)

6. 在 Name 中填入当前提供商的名字，随意，我填的是：`openrouter-mistral-7b-free`

![|550](attachments/Pasted%20image%2020241024151733.png)

7. 点击 Advance mode 后面的开关，使其打开

![|525](attachments/Pasted%20image%2020241024151942.png)

8. 向下拉，在 model 中，填入模型名字，即上面的：mistralai/mistral-7b-instruct:free

![|500](attachments/Pasted%20image%2020241024152011.png)
...
![|475](attachments/Pasted%20image%2020241024160926.png)
...
![|425](attachments/Pasted%20image%2020241024161647.png)
...
![|525](attachments/Pasted%20image%2020241024162433.png)

![|525](attachments/Pasted%20image%2020241024162521.png)

##### （6）、设置快捷键

1. 点击设置，然后选择快捷键，拉到最后，直到看到 Text Generator 开头的项

![|525](attachments/Pasted%20image%2020241024163034.png)
...
![|408](attachments/Pasted%20image%2020240709084030.png)
...
![|525](attachments/Pasted%20image%2020240709085956.png)
...
### ①、ltsc 系统安装 `.msixbundle` 或 `.msix` 应用包
...
![|575](attachments/Pasted%20image%2020250811083002.png)
...
> 
> [Genymotion-ARM-Translation_for_4.3.zip](attachments/Genymotion-ARM-Translation_for_4.3.zip)
> 
> [Genymotion-ARM-Translation_for_4.4.zip](attachments/Genymotion-ARM-Translation_for_4.4.zip)
> 
> [Genymotion-ARM-Translation_for_5.1.zip](attachments/Genymotion-ARM-Translation_for_5.1.zip)
> 
> [Genymotion-ARM-Translation_for_6.0.zip](attachments/Genymotion-ARM-Translation_for_6.0.zip)
> 
...
> 
> [Genymotion-ARM-Translation_for_8.0.zip](attachments/Genymotion-ARM-Translation_for_8.0.zip)
> 
> [Genymotion-ARM-Translation_for_9.0.zip](attachments/Genymotion-ARM-Translation_for_9.0.zip)
...
1. 原因：Genymotion 运行虚拟机是依靠 VirtualBox，但是，启动 Genymotion 虚拟机后，VirtualBox 会创建一个网络连接，问题出现在这里：网络连接未启动
2. 解决方案：
3. 在 Windows 下打开控制面板，按照路径：控制面板 --> 网络和 Internet --> 网络和共享中心 --> 更改适配器选项

![|675](attachments/Pasted%20image%2020231019152919.png)

4. 将被 `VirtualBox Host-Only Nerwork` 创建的所有连接关闭，然后打开最大的那个即可

![|650](attachments/Pasted%20image%2020231019152956.png)
...
2. 配置全局 Path：
	1. scrcpy 目录：`D:\app\devTools\Environments\Android\scrcpy-win64`
3. 手机开启 usb 调试
4. 电脑上打开命令行窗口，输入：`scrcpy` 即可连接
5. 如果有多个设备，使用 `--tcpip` 进行指定：`scrcpy --tcpip=192.168.1.21`

### ②、使用 Android SDK 的 adb

1. 下载解压：https://github.com/Genymobile/scrcpy/releases
2. 进入到解压目录，<font color="#d83931">删除以下 3 个文件</font>：`adb.exe`、`AdbWinApi.dll`、`AdbWinUsbApi.dll`
3. 设置 adb 环境变量：
	1. 变量名：`ADB`
	2. 变量值：`D:\app\devTools\Environments\Android\Android_SDK\platform-tools\adb.exe`，Android SDK 的 adb 目录
4. 3. 设置 Android SDK 环境变量：
	1. 变量名：`ANDROID_HOME`
	2. 变量值：`D:\app\devTools\Environments\Android\Android_SDK`，Android SDK 目录
5. 设置 scrcpy 环境变量：
	1. 变量名：`ANDROID_SCRCPY`
	2. 变量值：`D:\app\devTools\Environments\Android\scrcpy-win64`，Android SDK 的 adb 目录
6. 配置全局 Path：
	1. Android SDK 的 adb 目录：`%ANDROID_HOME%\platform-tools`
	2. scrcpy 目录：`%ANDROID_SCRCPY%`
7. 验证：
...
> 1. 官方文档：`https://github.com/Genymobile/scrcpy/blob/master/doc/shortcuts.md`
> 2. 在 Windows 中，MOD 一般代表 Alt 键
...
![|250](attachments/4A13FF6FFC900441630FD400F9D43B98.jpg)

3. 电脑端打开 DroidCamApp 软件，选择 USB 连接，选择自己的手机，填入端口号，点击 `Start` 进行连接

![|240](attachments/Pasted%20image%2020230420084259.png)

4. 连接成功后：

![|315](attachments/Pasted%20image%2020230420084501.png)

## 6、连接手机使其作为外放音响 1 SoundWire（不推荐）

> 电脑端：[SoundWire_Server_setup7-8-10.zip](attachments/SoundWire_Server_setup7-8-10.zip)
> 
> 安卓端：[SoundWire_Free.apk](attachments/SoundWire_Free.apk)
> 
> 推荐使用下面的 SyncAudio
...
![|418](attachments/Pasted%20image%2020231023141708.png)

3. 在 Input Select 选择输出音频，选一个未使用的
4. 之后再到Windows音量中修改输出设备，选择你在软件中勾选的输出源

![|413](attachments/Pasted%20image%2020231023141752.png)

5. 手机端在中间的方框中输入本地 IP，回车键即可连接

![|325](attachments/Pasted%20image%2020231023141820.png)

## 7、连接手机使其作为外放音响 2 SyncAudio

> 电脑端下载：[SyncAudio(同步听) 1.0.3.7z](attachments/SyncAudio%28%E5%90%8C%E6%AD%A5%E5%90%AC%29%201.0.3.7z)
