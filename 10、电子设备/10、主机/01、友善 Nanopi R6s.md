...
1. 点击服务器设置，然后在服务器设置下方输入名称如 `manual_proxy` 后点击添加

![|700](attachments/Pasted%20image%2020260731131501.png)

2. 点击添加后会弹出弹窗，大致这样设置即可：
    1. 启用：勾选
    2. 防火墙：不勾选，勾选后会开放给外网 WAN 访问
    3. 类型：混合（Mixed）
    4. 监听地址： `0.0.0.0`，监听所有 IPv4 地址
    5. 监听端口： 7890，或者其他随意的端口
    6. 用户名 / 密码：留空
    7. TCP 快速打开 / 多路径 TCP（MPTCP） / UDP 分片：不勾选
    8. UDP NAT 过期时间：默认即可
    9. 绑定接口：选择 lan 口
    10. 复用地址：不勾选

![|700](attachments/Pasted%20image%2020260731131737.png)

3. 配置完成后点击保存，保证新增的这一条启用被勾选了，然后勾选上方全局设置的启用，最后点击保存并应用

![|700](attachments/Pasted%20image%2020260731132715.png)

4. HomeProxy 服务器设置完毕后，直接在系统设置里开启代理、或者在软件里开启代理即可

![|700](attachments/Pasted%20image%2020260731133924.png)
