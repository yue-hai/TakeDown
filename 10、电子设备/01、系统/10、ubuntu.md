...
1. `du` (disk usage) 命令是 Linux 中用于评估文件或目录在磁盘上实际占用空间大小的核心工具
2. 它可以与 `df` (disk free) 配合使用：`df` 宏观地查看整个磁盘分区/文件系统的剩余容量，而 `du` 则从微观层面递归遍历文件树，精确找出到底是谁吃掉了我的硬盘容量

### ②、基本选项

1. `-h`：(Human-readable) 以人类易读的格式显示大小（自动转换为 K, M, G）
2. `-s`：(Summarize) 仅显示指定目录的总计大小，不展开列出其下的子目录详情
3. `-a`：(All) 不仅显示目录大小，同时列出所有单个文件的大小
4. `-c`：(Total) 在输出列表的最末尾，追加一行所有列出项的总计大小（Grand total）
5. `-d N` 或 `--max-depth=N`：指定递归遍历的最大层级深度。-d 1 表示只看当前目录下的第一层子目录，避免输出刷屏

### ③、高级选项

1. `--exclude=<PATTERN>`：在统计时排除匹配特定通配符的文件或目录（例如 `--exclude="*.log"`）
2. `-x`：(One file system) 仅统计当前文件系统上的数据，跳过挂载在此目录下的其他外部磁盘或网络存储（非常适合在 / 根目录下排查空间时使用）
3. `--time`：在输出空间大小的同时，显示该文件或目录最后一次被修改的时间戳
4. `-b / -k / -m`：强制统一以 Byte、KB 或 MB 为单位输出，屏蔽单位换算，非常适合用于 Shell 脚本的数值比对
5. `-L`：(Dereference) 跟踪符号链接。默认情况下 du 只计算快捷方式本身的大小（通常只有几个字节），加上此参数会计算该链接指向的真实目标文件大小

### ④、过滤和查询

1. `du` 命令原生采用的是遍历即输出模式。在 Unix 哲学下，我们通常通过管道符 | 将其与 `sort`、`grep`、`head` 等文本处理工具强强联合，实现高级查询。例如：
2. 统计当前目录下所有一层子目录的大小，并按人类易读单位从大到小逆序排列（最常用的排障连招）：

```shell
du -h --max-depth=1 | sort -rh
```

3. 统计整个项目目录的总大小，但排除掉吃硬盘的 .git 隐藏目录和 node_modules 依赖库：

```shell
du -sh --exclude=".git" --exclude="node_modules" .
```

4. 查找 /var/log 目录下最占空间的 5 个大文件或子目录：
...
2. 查看当前目录下所有文件和目录的大小，并在最底端输出一个总和：

```shell
du -ach
```

3. 查看指定目录的第一层目录，同时跳过已经挂载在 /mnt 等目录下的外部数据盘：

```shell
du -h -x --max-depth=1 /mnt/data | sort -rh
```

4. 查看当前目录的第一层目录，同时跳过已经挂载在 /mnt 等目录下的外部数据盘：

```shell
du -h -x --max-depth=1 ./ | sort -rh
```

5. 审查当前路径下的依赖包体积，并带上时间戳，方便判断哪些是陈旧的冗余数据：
