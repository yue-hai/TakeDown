...
1. 按下键盘上的 Win + R 键，输入 `compmgmt.msc` 并回车，打开计算机管理
2. 在左侧依次展开：系统工具 -> 本地用户和组 -> 用户
3. 在中间的空白处右键点击，选择 **新用户**
4. 填入你想要的用户名和密码
5. 极其关键的勾选项：
	1. 取消勾选 **用户下次登录时须更改密码**
	2. 勾选 **密码永不过期**
6. 点击 **创建**，然后关闭这个小窗口
7. 在用户列表中，双击刚建好的新用户，切换到 **隶属于** (Member Of) 选项卡
8. 点击左下方的 添加 -> 在输入框里输入 Administrators -> 点击右侧的 检查名称（名字会带上下划线） -> 点击确定
9. 现在的新用户已经是超级管理员了，点击应用并确定
...
1. 在服务器管理器中，点击左侧菜单栏的 **本地服务器** (Local Server)
2. 在右侧的主界面中，找到 **远程桌面** (Remote Desktop) 这一项，它现在的状态应该是 **已禁用** (Disabled)

![|700](attachments/Pasted%20image%2020260323103625.png)

3. 直接点击 **已禁用** 这几个蓝色的字，会弹出一个 **系统属性** 窗口

![|700](attachments/Pasted%20image%2020260323103710.png)

4. 在弹出的窗口中，选择 **允许远程连接到此计算机** (Allow remote connections to this computer)。此时可能会弹出一个防火墙提示，点击“确定”即可

![|700](attachments/Pasted%20image%2020260323103731.png)

5. 建议保留勾选下方的仅允许运行使用网络级别身份验证的远程桌面的计算机连接 (NLA)，这能大大增加安全性
6. 点击 **选择用户**，将想要允许使用远程桌面的用户加入

![|700](attachments/Pasted%20image%2020260323104010.png)
...
### ②、windows server 远程桌面不显示麦克风

#### Ⅰ、报错现象

1. 实体电脑上有音响和麦克风，并且都可以正常使用
2. 远程桌面连接时，音频、麦克风都打开了
3. 虚拟机上播放声音主电脑上能听到，并且并且设备里有播放设备
4. 但是被远程的机器里还是没有录制设备，无法使用实体电脑上的麦克风

![|325](attachments/Pasted%20image%2020260330145727.png)

![|362](attachments/Pasted%20image%2020260330145652.png)

#### Ⅱ、原因


1. 刚装好 windows Server 2016 时，RDP 默认处于 **用于管理的远程桌面** (Remote Desktop for Administration) 模式
2. 它的设计初衷仅仅是为了让 IT 管理员远程维护服务器（比如看日志、敲代码）
3. 为了绝对节省服务器的 CPU 计算能力和网络带宽，微软在系统底层直接阉割了复杂的多媒体双向传输协议。所以无论怎么改组策略允许录音，底层根本就没有加载对应的录制终结点驱动
4. 所以解决办法就是：安装 **远程桌面会话主机** (RDSH) 角色，这时系统会重构整个 RDP 协议栈，将其切换为虚拟桌面模式。在这个模式下，服务器认为要把它作为一台完整的日常办公电脑提供给普通员工使用，于是它加载了全套的音视频重定向驱动，从而让麦克风得以穿透

#### Ⅲ、解决

1. 打开 **服务器管理器** (Server Manager)

![|700](attachments/Pasted%20image%2020260330151023.png)

2. 在右上角的菜单中点击 **管理**，选择 **添加角色和功能**

![|700](attachments/Pasted%20image%2020260330151051.png)

3. 在向导窗口中，一直点击下一步，直到来到 **安装类型**，选择 **基于角色或基于功能的安装**

![|700](attachments/Pasted%20image%2020260330151146.png)

4. 点击下一步选择虚拟机，来到 **服务器角色** 列表区域，找到并勾选 **远程桌面服务** (Remote Desktop Services)
	1. 选中它后，如果有弹窗提示需要附带什么功能，直接点添加功能

![|700](attachments/Pasted%20image%2020260330151229.png)

5. 继续点击向导的下一步，在 **角色服务** 列表中，勾选 **远程桌面会话主机** (Remote Desktop Session Host)。

![|700](attachments/Pasted%20image%2020260330151304.png)

6. 确认后一直点击下一步，最后点击安装
7. 安装完成后，必须要重启整个虚拟机

![|700](attachments/Pasted%20image%2020260330151317.png)

### ③、windows server 远程桌面会话主机 120天的授权宽限期

> 1. 安装了 远程桌面会话主机 (RDSH)角色后，Windows Server 会进入 120天的授权宽限期
> 2. 120天之后，如果服务器没有配置 RDS 授权服务器和购买 CAL 许可证，将完全无法通过 RDP 登录这个虚拟机

#### Ⅰ、解决方式 1：无限重置 120 天宽限期

1. 在虚拟机按 `Win + R` 输入 `regedit` 打开注册表
2. 依次展开定位到：`HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server\RCM\GracePeriod`
3. 里面会有一个名字包含 `L$RTMTIMEBOMB` 的项（这就是 120 天倒计时炸弹）
4. 注意权限： 默认无法删除它。需要右键点击左侧树状图的 `GracePeriod` 文件夹，选择 权限 -> 高级，将顶部的 **所有者** 更改为当前管理员账号（Administrator），然后在权限里给 Administrators 组勾选 **完全控制**
5. 拥有权限后，直接把那个 `L$RTMTIMEBOMB...` 键值删除
6. 重启虚拟机，120 天倒计时就会重新开始（可以无限次这么干）

#### Ⅱ、解决方式 2：配置本地 RDS 授权

##### （1）、安装 **授权服务器** 组件

1. 打开 服务器管理器 -> 添加角色和功能 -> 远程桌面服务
2. 再额外安装一个 **远程桌面授权** (RD Licensing) 角色，然后重启虚拟机

![|700](attachments/Pasted%20image%2020260330154808.png)

##### （2）、激活授权服务器并生成 500 张门票

1. 按 `Win + S` 搜索，或者在管理工具中找到并打开 **远程桌面授权管理器** (RD Licensing Manager)

![|700](attachments/Pasted%20image%2020260330155353.png)

2. 这时会看到左侧列表里有服务器名字，旁边是一个带红叉的图标（表示未激活）

![|700](attachments/Pasted%20image%2020260330155409.png)

3. 右键点击服务器名字 -> 选择 **激活服务器**

![|700](attachments/Pasted%20image%2020260330155436.png)

4. 弹出向导，连接方法保持默认的 **自动连接** (要求 Internet 连接)，下一步

![|700](attachments/Pasted%20image%2020260330155500.png)

5. 随便填写公司的信息（名字、姓氏、公司名随便填，比如 test/test/IT），国家选中国，下一步，其他可选信息留空直接下一步，完成激活

![|700](attachments/Pasted%20image%2020260330155542.png)

![|700](attachments/Pasted%20image%2020260330155604.png)

6. 激活完成后，默认会勾选 **立即启动许可证安装向导**，下一步

![|700](attachments/Pasted%20image%2020260330155622.png)

7. 在弹出的安装许可证向导中，许可证计划 下拉表里，一定要选择 **企业协议** (Enterprise Agreement)，下一步

![|700](attachments/Pasted%20image%2020260330155650.png)

8. 关键步骤 -> 输入协议号码： 输入公共企业协议号码，例如 6565792 （或 4954438、4526017），下一步

![|700](attachments/Pasted%20image%2020260330155723.png)

9. 产品版本选择 Windows Server 2016，许可证类型选择 **RDS 每用户 CAL** (RDS Per User CAL)，数量输入 500，然后下一步到完成

![|700](attachments/Pasted%20image%2020260330155806.png)

10. 现在回到管理器界面，红叉变成了绿叉（或者正常图标），右侧显示有 500 个有效的 **按用户** 授权了

![|700](attachments/Pasted%20image%2020260330155846.png)

##### （3）、告诉主机去哪里取号

1. 按 `Win + R` 键，输入 `gpedit.msc` 打开本地组策略编辑器
2. 依次展开：计算机配置 -> 管理模板 -> Windows 组件 -> 远程桌面服务 -> 远程桌面会话主机 -> 授权
3. 双击打开 **使用指定的远程桌面许可证服务器**
	1. 选择 已启用
	2. 在下方的输入框中输入：127.0.0.1 （意思就是找自己取号）
	3. 点击确定

![|700](attachments/Pasted%20image%2020260330160031.png)

4. 双击打开同目录下的 **设置远程桌面授权模式**
	1. 选择 已启用
	2. 在下方下拉菜单中选择 **按用户** (Per User)
	3. 点击确定

![|700](attachments/Pasted%20image%2020260330160056.png)

##### （4）、检查是否成功

1. 按 `Win + R`，输入 `cmd`，运行 `gpupdate /force` 刷新策略
2. 按 `Win + S` 搜索，或者在管理工具中找到并打开 **远程桌面授权诊断程序** (RD Licensing Diagnoser)

![|700](attachments/Pasted%20image%2020260330160558.png)

3. 如果看到里面是绿色的勾 ✅，显示未发现问题，可用许可证数量显示 500，且没有 120 天到期警告，那么它的远程桌面现在就是永久可用的了

![](attachments/Pasted%20image%2020260330161723.png)
...
## 4、通过代理 url 获取代理 Ip 和端口

> 代理 URL 指向一个叫做 PAC (Proxy Auto-Configuration) 的脚本文件
> 
> 可以把它理解成一个智能代理脚本，它会根据要访问的网址，动态地决定是直接连接还是通过某个代理服务器连接
> 
> 但是有的软件、插件不支持

1. 在浏览器中访问这个 PAC 文件的 URL，比如：http://proxy.yuehai.cn/
2. 它会下载或显示一个 `.pac` 文件，这个文件其实是 JavaScript 脚本。用记事本或任何文本编辑器打开它
3. 在文件内容里搜索关键词 `PROXY`，一般在文件最后，可能会找到类似 `PROXY 10.20.30.40:8080` 这样的代码。这里的 10.20.30.40 就是主机名，8080 就是端口号
4. 回到软件或插件中，输入这个 Ip 和端口即可
